...
macro (COLLECT_AND_INSTALL_OCCT_HEADER_FILES ROOT_TARGET_OCCT_DIR OCCT_BUILD_TOOLKITS OCCT_COLLECT_SOURCE_DIR OCCT_INSTALL_DIR_PREFIX)
  # Remove all API header files
  foreach (OCCT_MODULE ${OCCT_MODULES})
    set (OCCT_MODULE_API_HEADER_FILE ${ROOT_TARGET_OCCT_DIR}/${OCCT_INSTALL_DIR_PREFIX}/${OCCT_MODULE}API.h)
    if (EXISTS "${OCCT_MODULE_API_HEADER_FILE}")
      file (REMOVE "${OCCT_MODULE_API_HEADER_FILE}")
    endif()
    foreach(OCCT_TOOLKIT ${${OCCT_MODULE}_TOOLKITS})
      set (OCCT_TOOLKIT_API_HEADER_FILE ${ROOT_TARGET_OCCT_DIR}/${OCCT_INSTALL_DIR_PREFIX}/${OCCT_TOOLKIT}API.h)
      if (EXISTS "${OCCT_TOOLKIT_API_HEADER_FILE}")
        file (REMOVE "${OCCT_TOOLKIT_API_HEADER_FILE}")
      endif()
    endforeach()
  endforeach()
  ...
  foreach (OCCT_USED_TOOLKIT ${OCCT_BUILD_TOOLKITS})
    ...
    # Get packages parent toolkits
    set (OCCT_${OCCT_USED_TOOLKIT}_PACKAGES ${OCCT_TOOLKIT_PACKAGES})
    foreach (OCCT_PACKAGE ${OCCT_TOOLKIT_PACKAGES})
      set (OCCT_PACKAGE_${OCCT_PACKAGE}_PARENT ${OCCT_USED_TOOLKIT})
    endforeach()

    # Get toolkits parent modules
    foreach (MODULE ${OCCT_MODULES})
      set (TOOLKITS ${${MODULE}_TOOLKITS})
      list (FIND TOOLKITS ${OCCT_USED_TOOLKIT} FOUND)
      if (NOT ${FOUND} EQUAL -1)
        set (OCCT_${OCCT_USED_TOOLKIT}_PARENT ${MODULE})
      endif()
    endforeach(MODULE)
  endforeach()
  ...
  foreach (OCCT_PACKAGE ${OCCT_USED_PACKAGES})
    if (NOT "x${BUILD_API_HEADER_FILES}" STREQUAL "xDo not generate")
      set (OCCT_${OCCT_PACKAGE_${OCCT_PACKAGE}_PARENT}_HEADER_FILES "${OCCT_${OCCT_PACKAGE_${OCCT_PACKAGE}_PARENT}_HEADER_FILES}\n// ${OCCT_PACKAGE}")
    endif()
  ...
  string(TIMESTAMP CURRENT_TIME "%H:%M:%S")
  message (STATUS "Info: \(${CURRENT_TIME}\) Creating API headers in inc folder...")

  if ( "x${BUILD_API_HEADER_FILES}" STREQUAL "xOne per toolkit" OR
       "x${BUILD_API_HEADER_FILES}" STREQUAL "xOne per toolkit and module")
    foreach (OCCT_USED_TOOLKIT ${OCCT_BUILD_TOOLKITS})
      set (OCCT_API_HEADER_FILE_CONTENT "${OCCT_${OCCT_USED_TOOLKIT}_HEADER_FILES}")
      configure_file("${CMAKE_SOURCE_DIR}/adm/templates/headerAPI.in" "${ROOT_TARGET_OCCT_DIR}/${OCCT_INSTALL_DIR_PREFIX}/${OCCT_USED_TOOLKIT}API.h" @ONLY)
      install (FILES "${ROOT_TARGET_OCCT_DIR}/${OCCT_INSTALL_DIR_PREFIX}/${OCCT_USED_TOOLKIT}API.h" DESTINATION "${INSTALL_DIR_INCLUDE}")
    endforeach()
  endif()

  if ( "x${BUILD_API_HEADER_FILES}" STREQUAL "xOne per toolkit and module")
    set (OCCT_USED_MODULES)
    foreach (OCCT_USED_TOOLKIT ${OCCT_BUILD_TOOLKITS})
      list (APPEND OCCT_USED_MODULES ${OCCT_${OCCT_USED_TOOLKIT}_PARENT})
      set (${OCCT_${OCCT_USED_TOOLKIT}_PARENT}_HEADER_FILES "${${OCCT_${OCCT_USED_TOOLKIT}_PARENT}_HEADER_FILES}\n#include \"${OCCT_USED_TOOLKIT}API.h\"")
    endforeach()

    list (REMOVE_DUPLICATES OCCT_USED_MODULES)
    foreach (OCCT_USED_MODULE ${OCCT_USED_MODULES})
      set (OCCT_API_HEADER_FILE_CONTENT "${${OCCT_USED_MODULE}_HEADER_FILES}")
      configure_file("${CMAKE_SOURCE_DIR}/adm/templates/headerAPI.in" "${ROOT_TARGET_OCCT_DIR}/${OCCT_INSTALL_DIR_PREFIX}/${OCCT_USED_MODULE}API.h" @ONLY)
      install (FILES "${ROOT_TARGET_OCCT_DIR}/${OCCT_INSTALL_DIR_PREFIX}/${OCCT_USED_MODULE}API.h" DESTINATION "${INSTALL_DIR_INCLUDE}")
    endforeach()
  endif()

  if ( "x${BUILD_API_HEADER_FILES}" STREQUAL "xOne per module")
    set (OCCT_USED_MODULES)
    set (LINE_BREAK "")
    foreach (OCCT_USED_TOOLKIT ${OCCT_BUILD_TOOLKITS})
      list (APPEND OCCT_USED_MODULES ${OCCT_${OCCT_USED_TOOLKIT}_PARENT})
      set (${OCCT_${OCCT_USED_TOOLKIT}_PARENT}_HEADER_FILES "${${OCCT_${OCCT_USED_TOOLKIT}_PARENT}_HEADER_FILES}${LINE_BREAK}\n// ${OCCT_USED_TOOLKIT}\n${OCCT_${OCCT_USED_TOOLKIT}_HEADER_FILES}")
      set (LINE_BREAK "\n")
    endforeach()
