...
static int tinspector (Draw_Interpretor& di, Standard_Integer theArgsNb, const char** theArgs)
{
  ...
  for (Standard_Integer anIt = 1; anIt < theArgsNb; ++anIt)
  {
    ...
    else if (aParam.IsEqual ("-report"))
    {
      anIt++;
      Handle(Draw_Report) aReport = Handle(Draw_Report)::DownCast (Draw::Get (theArgs[anIt]));
      if (!aReport.IsNull())
      {
        NCollection_List<Handle(Standard_Transient)> aMessageViewParameters;
        aParameters.Find ("TKMessageView", aMessageViewParameters);
        aMessageViewParameters.Append (aReport->GetReport());
        aParameters.Bind ("TKMessageView", aMessageViewParameters);
      }
...
void ToolsDraw::Commands(Draw_Interpretor& theCommands)
{
  ...
  theCommands.Add ("tinspector",
    ...
    "\n\t\t:  -report gives the report to MessageView plugin;"
