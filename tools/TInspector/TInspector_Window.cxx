...
#include <inspector/TInspector_ReportCallBack.hxx>
...
const int TINSPECTOR_DEFAULT_WIDTH = 860;//650;
const int TINSPECTOR_DEFAULT_HEIGHT = 550;//350;
const int TINSPECTOR_DEFAULT_POSITION_X = 450;//200;
const int TINSPECTOR_DEFAULT_POSITION_Y = 90;//60;

class TInspector_MainWindow : public QMainWindow
{
public:
  TInspector_MainWindow(TInspector_Window* theWindow, QWidget* theParent)
    : QMainWindow (theParent), myWindow(theWindow) {}
  ~TInspector_MainWindow() {}
...
{
  myMainWindow = new TInspector_MainWindow(this, 0);
  ...
  myParameters->SetCallBack(new TInspector_ReportCallBack(this));
...
void TInspector_Window::UpdateContent()
{
  ...
  const NCollection_List<TCollection_AsciiString>& aPlugins = myParameters->GetImmediateUpdatedPlugins();
  int aToolId;
  for (NCollection_List<TCollection_AsciiString>::Iterator anIterator (aPlugins);
       anIterator.More(); anIterator.Next())
  {
    if (!findPlugin (anIterator.Value(), anInfo, aToolId))
      continue;
