                                             ...
                                             const Standard_Boolean theOnlyCount)
{
  ...
  if (!theOnlyCount)
  {
    Standard_Size aPosBefore = (Standard_Size)theOStream.tellp();

    theOStream.write ((char*)&t, sizeof(Standard_Integer));

    Standard_Size aPosAfter = (Standard_Size)theOStream.tellp();

    if (aPosAfter - aPosBefore != sizeof(Standard_Integer))
    {
                                              ...
                                              const Standard_Boolean         theOnlyCount)
{
  ...
  if (anAsciiStrLen > 0 && !theOnlyCount)
  {
    ...
    if (Standard_Integer(aPosAfter - aPosBefore) != anAsciiStrLen)
    {
      Storage_StreamWriteError::Raise();
    }
  }
                                                      ...
                                                      const Standard_Boolean            theOnlyCount)
{
  ...
  if (anExtStrLen > 0 && !theOnlyCount)
  {
    ...
    if (Standard_Integer(aPosAfter - aPosBefore) != sizeof(Standard_ExtCharacter)*theString.Length())
    {
      Storage_StreamWriteError::Raise();
    }
  }
