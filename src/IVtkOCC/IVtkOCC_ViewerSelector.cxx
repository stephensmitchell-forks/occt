...
Standard_Boolean  IVtkOCC_ViewerSelector::Update (const IVtk_IView::Handle& theView)
{
  ...
  if (anIt <= 13 || (myPrevCenter[0] != myCenter[0]) || (myPrevCenter[1] != myCenter[1]))
  {
    ...
    // Get rotation matrix
    gp_Dir aYpers (myCoeff[3], myCoeff[4], myCoeff[5]);
    gp_Dir aZpers (myCoeff[6], myCoeff[7], myCoeff[8]);
    gp_Dir aXpers = aYpers.Crossed (aZpers);
    gp_Mat aMatrix;
    aMatrix.SetRows (aXpers.XYZ(), aYpers.XYZ(), aZpers.XYZ());

    // Get translation vector
    gp_XYZ aLoc (myCoeff[0], myCoeff[1], myCoeff[2]);
    aLoc.Multiply (aMatrix);
    aLoc.Reverse();

    gp_Mat matScale (myCoeff[11], 0, 0, 0, myCoeff[12], 0, 0, 0, myCoeff[13]);
    aMatrix.Multiply (matScale);

    gp_Trsf aTrsf;
    gp_Quaternion aRotation (aMatrix);
    aTrsf.SetRotation(aRotation);
    aTrsf.SetTranslation (aLoc);

    myPrj = new Select3D_Projector (aTrsf, isPerspective, myCoeff[9]);
  }
