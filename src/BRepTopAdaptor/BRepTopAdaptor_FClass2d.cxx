...
//purpose  : 
//=======================================================================
  BRepTopAdaptor_FClass2d::BRepTopAdaptor_FClass2d() : Toluv(Precision::PConfusion()),
                                                       Umin(RealLast()),
                                                       Vmin(RealLast()),
                                                       Umax(-Umin),
                                                       Vmax(-Vmin)
  ...
  for (; aExpF.More(); aExpF.Next()) {
    ...
    //
      ...
      if (nbpnts>3) {
        ...
        // MaxAngle==3.0 is chosen in order to except faces with bending p-curves
        // (such faces will be classified with BRepClass_FaceClassifier only).
        const Standard_Boolean isDUGood = ((Umax - Umin) > 1.0e-3),
                               isDVGood = ((Vmax - Vmin) > 1.0e-3);
        //
        if ((isDUGood && isDVGood && (aMaxAngle > 3.0)) ||
...
TopAbs_State BRepTopAdaptor_FClass2d::PerformInfinitePoint() const
{
  ...
  const Standard_Real aMaxTol = BRep_Tool::MaxTolerance(Face, TopAbs_VERTEX);
