...
void BRepAlgoAPI_BuilderAlgo::BuildResult()
{
  ...
  myBuilder->SetToFillHistory(Standard_True);
                                             ...
                                             const Standard_Real    theAngularTol)
{
  ...
  myHistory->Merge(anUnifier.History());
}
//=======================================================================
//function : Modified
//purpose  : 
...
{
  return (myHistory ? myHistory->IsRemoved(theS) : Standard_False);
}
//=======================================================================
...
{
  return (myHistory ? myHistory->HasModified() : Standard_False);
}
//=======================================================================
...
{
  return (myHistory ? myHistory->HasGenerated() : Standard_False);
}
//=======================================================================
//function : HasDeleted
//purpose  : 
...
{
  return (myHistory ? myHistory->HasRemoved() : Standard_False);
}
//=======================================================================
...
const TopTools_ListOfShape& BRepAlgoAPI_BuilderAlgo::SectionEdges()
{
  ...
  // Iterate on all faces and ask for Generated shapes
  const BOPDS_PDS& pDS = myDSFiller->PDS();
  const Standard_Integer aNbS = pDS->NbSourceShapes();
  for (Standard_Integer i = 0; i < aNbS; ++i)
  {
    ...
    // Get all generated elements if any
    const TopTools_ListOfShape& aLE = myHistory->Generated(aSI.Shape());
    // Get edges only
    for (TopTools_ListIteratorOfListOfShape itLE(aLE); itLE.More(); itLE.Next())
    {
      const TopoDS_Shape& aSE = itLE.Value();
      if (aSE.ShapeType() == TopAbs_EDGE && aMFence.Add(aSE))
