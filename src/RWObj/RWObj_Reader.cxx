...
namespace
{
  ...
  bool isClockwisePolygon (const Handle(BRepMesh_DataStructureOfDelaun)& theMesh,
                           const BRepMesh::Array1OfInteger& theIndexes)
  {
    Standard_Real aPtSum = 0;
    const Standard_Integer aNbElemNodes = theIndexes.Size();
    for (Standard_Integer aNodeIter = theIndexes.Lower(); aNodeIter <= theIndexes.Upper(); ++aNodeIter)
    {
      Standard_Integer aNodeNext = theIndexes.Lower() + ((aNodeIter + 1) % aNbElemNodes);
...
// Purpose  :
// ================================================================
Standard_Boolean RWObj_Reader::Read (const char* theFile,
                                     const Handle(Message_ProgressIndicator)& theProgress)
{
  ...
  addSubMesh (myActiveSubMesh, RWObj_SubMeshReason_NewObject);

  // determine file location to load associated files
  OSD_Path aPath (theFile);
  aPath.SetName ("");
  aPath.SetExtension ("");
  aPath.SystemName (myFolder);
  ...
  if (myNbElemsBig != 0)
  {
    Message::DefaultMessenger()->Send (TCollection_AsciiString("OBJ reader, ") + myNbElemsBig + " polygon(s) have been split into triangles", Message_Warning);
  }
...
void RWObj_Reader::pushIndices (const char* thePos)
{
  ...
  for (Standard_Integer aNode = 0;; ++aNode)
  {
    ...
    if (a3Indices[0] < -1)
    {
      a3Indices[0] += myVerts.Upper() + 2;
    }
    if (a3Indices[1] < -1)
    {
      a3Indices[1] += myVertParams.Upper() + 2;
    }
    if (a3Indices[2] < -1)
    {
      a3Indices[2] += myNorms.Upper() + 2;
    }
    ...
    if (!myPackedIndices.Find (a3Indices, anIndex))
    {
      ...
      if (a3Indices[0] < myVerts.Lower() || a3Indices[0] > myVerts.Upper())
      {
        myToAbort = true;
        Message::DefaultMessenger()->Send (TCollection_AsciiString("Invalid OBJ syntax at line ") + myNbLines, Message_Fail);
        return;
      }

      anIndex = addSubMeshNode (myVerts.Value (a3Indices[0]));
      myPackedIndices.Bind (a3Indices, anIndex);
      if (a3Indices[1] >= 0)
      {
        if (myVertParams.IsEmpty())
        {
          Message::DefaultMessenger()->Send (TCollection_AsciiString("Invalid OBJ syntax at line ") + myNbLines
                                             + ": UV index is specified but no UV nodes are defined!", Message_Warning);
        }
        else if (a3Indices[1] < myVertParams.Lower() || a3Indices[1] > myVertParams.Upper())
        {
          Message::DefaultMessenger()->Send (TCollection_AsciiString("Invalid OBJ syntax at line ") + myNbLines + ": UV index is out of range!", Message_Warning);
          addSubMeshNodeUV (Graphic3d_Vec2 (0.0f, 0.0f));
        }
        else
        {
          addSubMeshNodeUV (myVertParams.Value (a3Indices[1]));
        }
      }
      if (a3Indices[2] >= 0)
      {
        ...
        else if (a3Indices[2] < myNorms.Lower() || a3Indices[2] > myNorms.Upper())
        {
          Message::DefaultMessenger()->Send (TCollection_AsciiString("Invalid OBJ syntax at line ") + myNbLines + ": Normal index is out of range!", Message_Warning);
          addSubMeshNormal (Graphic3d_Vec3 (0.0f, 0.0f, 1.0f));
        }
        else
        {
          addSubMeshNormal (myNorms.Value (a3Indices[2]));
        }
      }
    }
  ...
  if (aNbElemNodes == 3)
  {
    ...
    addSubMeshElement (myCurrElem[0], myCurrElem[1], myCurrElem[2], -1);
  }
  else if (aNbElemNodes == 4)
  {
    myMemEstim += sizeof(Graphic3d_Vec4i);
    addSubMeshElement (myCurrElem[0], myCurrElem[1], myCurrElem[2], myCurrElem[3]);
  }
  else
  {
...
Standard_Integer RWObj_Reader::triangulatePolygonFan (const NCollection_Array1<Standard_Integer>& theIndices)
{
  ...
  for (Standard_Integer aNodeIter = 0; aNodeIter < aNbElemNodes - 2; ++aNodeIter)
  {
    ...
    addSubMeshElement (aTriNodes[0], aTriNodes[1], aTriNodes[2], -1);
  }
...
// Purpose  :
//================================================================
Graphic3d_Vec3 RWObj_Reader::polygonCenter (const NCollection_Array1<Standard_Integer>& theIndices)
{
  Graphic3d_Vec3 aCenter (0, 0, 0);
  ...
  {
    aCenter = getSubMeshNode (theIndices.Value (theIndices.Lower() + 0))
            + getSubMeshNode (theIndices.Value (theIndices.Lower() + 2));
    aCenter /= 2.0f;
    return aCenter;
  }
  ...
  {
    aCenter += getSubMeshNode (aPntIter.Value());
  }

  aCenter /= (Standard_ShortReal )theIndices.Size();
  return aCenter;
}
...
// Purpose  :
//================================================================
Graphic3d_Vec3 RWObj_Reader::polygonNormal (const NCollection_Array1<Standard_Integer>& theIndices)
{
  const Graphic3d_Vec3 aCenter = polygonCenter (theIndices);
  Graphic3d_Vec3 aMaxDir = getSubMeshNode (theIndices.First()) - aCenter;
  Graphic3d_Vec3 aNormal = Graphic3d_Vec3::Cross (getSubMeshNode (theIndices.Last()) - aCenter, aMaxDir);
  for (int aPntIter = theIndices.Lower(); aPntIter < theIndices.Upper(); ++aPntIter)
  {
    const Graphic3d_Vec3 aTmpDir2 = getSubMeshNode (theIndices.Value (aPntIter + 1)) - aCenter;
    if (aTmpDir2.SquareModulus() > aMaxDir.SquareModulus())
    {
      aMaxDir = aTmpDir2;
    }

    const Graphic3d_Vec3 aTmpDir1 = getSubMeshNode (theIndices.Value (aPntIter)) - aCenter;
    Graphic3d_Vec3 aDelta = Graphic3d_Vec3::Cross (aTmpDir1, aTmpDir2);
...
Standard_Integer RWObj_Reader::triangulatePolygon (const NCollection_Array1<Standard_Integer>& theIndices)
{
  ...
  const Graphic3d_Vec3 aPolygonNorm = polygonNormal (theIndices);
  ...
  gp_XYZ aXDir;
  {
    ...
    aXDir.ChangeCoord (aI1) =  aPolygonNorm[aI2 - 1];
    aXDir.ChangeCoord (aI2) = -aPolygonNorm[aI1 - 1];
  }
  const gp_XYZ aYDir = gp_XYZ (aPolygonNorm.x(), aPolygonNorm.y(), aPolygonNorm.z()) ^ aXDir;
  ...
  BRepMesh::Array1OfInteger anIndexes (0, aNbElemNodes - 1);
  for (Standard_Integer aNodeIter = 0; aNodeIter < aNbElemNodes; ++aNodeIter)
  {
    ...
    const Graphic3d_Vec3 aVec3 = getSubMeshNode (aNodeIndex);
    const gp_XYZ aPnt3d (aVec3.x(), aVec3.y(), aVec3.z());
    gp_XY aPnt2d (aXDir * aPnt3d, aYDir * aPnt3d);
    BRepMesh_Vertex aVertex (aPnt2d, aNodeIndex, BRepMesh_Frontier);
    anIndexes.ChangeValue (aNodeIter) = aMeshStructure->AddNode (aVertex);
  }
  ...
  try
  {
    ...
    for (BRepMesh::MapOfInteger::Iterator aTriIter (aTriangles); aTriIter.More(); aTriIter.Next())
    {
      ...
      addSubMeshElement (aVertex1.Location3d(), aVertex2.Location3d(), aVertex3.Location3d(), -1);
      ++aNbTrisAdded;
    }
    return aNbTrisAdded;
  }
  ...
  {
    Message::DefaultMessenger()->Send (TCollection_AsciiString ("Exception raised during polygon split\n[") + theFailure.GetMessageString() + "]", Message_Warning);
  }
  return triangulatePolygonFan (theIndices);
}
...
{
  if (!RWObj_Tools::ReadName (theObjectName, myActiveSubMesh.Object))
  {
...
{
  if (!RWObj_Tools::ReadName (theGroupName, myActiveSubMesh.Group))
  {
    // empty group name is OK
  }
  addSubMesh (myActiveSubMesh, RWObj_SubMeshReason_NewGroup);
  myPackedIndices.Clear(); // vertices might be duplicated after this point...
}
...
{
  RWObj_Tools::ReadName (theSmoothGroupIndex, myActiveSubMesh.SmoothGroup);
  if (myActiveSubMesh.SmoothGroup == "off"
   || myActiveSubMesh.SmoothGroup == "0")
  {
    myActiveSubMesh.SmoothGroup.Clear();
  }
  addSubMesh (myActiveSubMesh, RWObj_SubMeshReason_NewSmoothGroup);
  myPackedIndices.Clear(); // vertices might be duplicated after this point...
}
...
{
  if (!RWObj_Tools::ReadName (theMaterialName, myActiveSubMesh.Material))
  {
    // empty material name is allowed by specs
  }
  else if (!myMaterials.IsBound (myActiveSubMesh.Material))
  {
    Message::DefaultMessenger()->Send (TCollection_AsciiString("Use of undefined OBJ material at line ") + myNbLines, Message_Warning);
    return;
  }

  // implicitly create a new group to split materials
  addSubMesh (myActiveSubMesh, RWObj_SubMeshReason_NewMaterial);
  myPackedIndices.Clear(); // vertices might be duplicated after this point...
}
...
void RWObj_Reader::readMaterialLib (const char* theFileName)
{
  ...
  {
    Message::DefaultMessenger()->Send (TCollection_AsciiString("Invalid OBJ syntax at line ") + myNbLines, Message_Warning);
    return;
  }

  RWObj_MtlReader aMatReader (myMaterials);
  aMatReader.Read (myFolder, aMatPath);
}
