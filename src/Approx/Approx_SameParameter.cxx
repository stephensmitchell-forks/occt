...
const Standard_Real Approx_SameParameter::myDeltaMin = Precision::PConfusion();
...
void Approx_SameParameter::Build(const Standard_Real Tolerance)
{
  ...
  {
    const Standard_Real aMult = 1.5; // To be tolerant to discrete tolerance computing.
    myTolReached = aMult * sqrt(aMaxSqDeviation);
    return;
  }
  ...
  if (!myDone)
  {
    ...
    Standard_Real anApproxTol = ComputeTolReached(myC3d,aData.myCOnS,myNbSamples);
...
//purpose  : Sub-method in Build.
//=======================================================================
void Approx_SameParameter::Interpolate(const Approx_SameParameter_Data & theData,
