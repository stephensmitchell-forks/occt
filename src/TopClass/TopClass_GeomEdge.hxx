...
public:
  ...
  //! Recomputes thePt on the surface mySurf to 3D-point and finds
  //! its distance to 3D-curve myC3D.
  //! thePar0 is used as initial parameter for search.
  //! Returns negative value in case of fail.
  Standard_EXPORT Standard_Real Distance3D(const gp_Pnt2d &thePt,
                                           const Standard_Real thePar0) const;

  //! Looks for thePntOnC, which is nearest point in
  //! the 2D-curve myC2D to thePt. theParOnC is the 
  //! parameter on myC2D respected to thePntOnC.
  //! Returns the distance between thePt and thePntOnC.
  //! Returns negative value in case of fail.
  ...
  const TopAbs_Orientation GetOrientation() const
  {
    return myOri;
  }
  ...
  const Standard_Boolean IsPseudo() const
  {
