...
const Handle(Geom_Surface)& BRep_Tool::Surface(const TopoDS_Face& F,
                                               TopLoc_Location& L)
{
  const BRep_TFace* TF = static_cast<const BRep_TFace*>(F.TShape().operator->());
...
Handle(Geom_Surface) BRep_Tool::Surface(const TopoDS_Face& F)
{
  ...
  if (!L.IsIdentity()) {
    ...
    Geom_Surface* aGS = static_cast<Geom_Surface*>(aCopy.operator->());
                                            ...
                                            Standard_Real& Last)
{
  ...
  const BRep_TEdge* TE = static_cast<const BRep_TEdge*>(E.TShape().operator->());
  BRep_ListIteratorOfListOfCurveRepresentation itcr(TE->Curves()); 

  while (itcr.More()) {
    const Handle(BRep_CurveRepresentation)& cr = itcr.Value();
    if (cr->IsCurve3D()) {
      const BRep_Curve3D* GC = static_cast<const BRep_Curve3D*>(cr.operator->());
                                     ...
                                     Standard_Real& Last)
{
  ...
  if ( !C.IsNull() ) {
    if ( !L.IsIdentity() ) {
      ...
      Geom_Curve* aGC = static_cast<Geom_Curve*>(aCopy.operator->());
                                                   ...
                                                   TopLoc_Location&   L)
{
  ...
  while (itcr.More()) {
    const Handle(BRep_CurveRepresentation)& cr = itcr.Value();
    if (cr->IsPolygon3D()) {
      const BRep_Polygon3D* GC = static_cast<const BRep_Polygon3D*>(cr.operator->());
      L = E.Location() * GC->Location();
      return GC->Polygon3D();
    }
    itcr.Next();
                                               ...
                                               Standard_Real& Last)
{
  ...
  while (itcr.More()) {
    ...
    if (cr->IsCurveOnSurface(S,loc)) {
      const BRep_GCurve* GC = static_cast<const BRep_GCurve*>(cr.operator->());
      GC->Range(First,Last);
                                 ...
                                 TopLoc_Location&        L)
{
  ...
  while (itcr.More()) {
    ...
    if (cr->IsPolygonOnSurface()) {
      const BRep_PolygonOnSurface* PS = static_cast<const BRep_PolygonOnSurface*>(cr.operator->());
      P = PS->Polygon();
      S = PS->Surface();
  ...
  while (itcr.More()) {
    ...
    if (cr->IsPolygonOnTriangulation()) {
      const BRep_PolygonOnTriangulation* PT =
        static_cast<const BRep_PolygonOnTriangulation*>(cr.operator->());
                       ...
                       Standard_Real& Last)
{
  ...
  while (itcr.More()) {
    const Handle(BRep_CurveRepresentation)& cr = itcr.Value();
    if (cr->IsCurve3D()) {
      const BRep_Curve3D* CR = static_cast<const BRep_Curve3D*>(cr.operator->());
      ...
      const BRep_GCurve* CR = static_cast<const BRep_GCurve*>(cr.operator->());
      First = CR->First(); 
      Last = CR->Last();
      break;
    }
                       ...
                       Standard_Real& Last)
{
  ...
  while (itcr.More()) {
    const Handle(BRep_CurveRepresentation)& cr = itcr.Value();
    if (cr->IsCurveOnSurface(S,l)) {
      const BRep_CurveOnSurface* CR = static_cast<const BRep_CurveOnSurface*>(cr.operator->());
      CR->Range(First,Last);
      break;
    }
    itcr.Next();
    ...
    if (cr->IsCurveOnSurface(S,l)) {
      if (cr->IsCurveOnClosedSurface() && Eisreversed)
      {
        const BRep_CurveOnClosedSurface* CR =
          static_cast<const BRep_CurveOnClosedSurface*>(cr.operator->());
        CR->UVPoints2(PFirst, PLast);
      }
      else
      {
        const BRep_CurveOnSurface* CR =
          static_cast<const BRep_CurveOnSurface*>(cr.operator->());
                             ...
                             const gp_Pnt2d& PLast)
{
  ...
  while (itcr.More()) {
    ...
    if (cr->IsCurveOnSurface(S,l)) {
      if (cr->IsCurveOnClosedSurface() && Eisreversed)
      {
        BRep_CurveOnClosedSurface* CS = static_cast<BRep_CurveOnClosedSurface*>(cr.operator->());
        CS->SetUVPoints2(PFirst, PLast);
      }
      else
      {
        BRep_CurveOnSurface* CS = static_cast<BRep_CurveOnSurface*>(cr.operator->());
...
{
  const BRep_TVertex* TV = static_cast<const BRep_TVertex*>(V.TShape().operator->());
...
{
  const BRep_TVertex* aTVert = static_cast<const BRep_TVertex*>(V.TShape().operator->());
