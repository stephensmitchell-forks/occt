					       ...
					       const Standard_Boolean firstCall)
{
  ...
  Standard_Real aTolV1 = 0., aTolV2 = 0.;
  ...
  if (firstCall && ( !isResEdge || !isSamePar || tolReached > myTolerance)) {
    ...
    if( !s_edge.IsNull())
    {
      ...
      second_ok = (BRep_Tool::SameParameter(s_edge) && tolReached_2 < tolReached - Precision::Confusion());
      if( second_ok)
      {
        edge = s_edge;
        whichSec = whichSecn;
        ...
        if (isResEdge)
        {
          ...
          aTV1->Tolerance(aTolV1);
          Handle(BRep_TVertex)  aTV2 = Handle(BRep_TVertex)::DownCast(aV2.TShape());
          aTV2->Tolerance(aTolV2);
          BRepLib::UpdateInnerTolerances(edge);
        }
      }
    }
