...
BRepTopAdaptor_FClass2d& IntTools_Context::FClass2d(const TopoDS_Face& aF)
{
  ...
  if (!myFClass2dMap.IsBound(aF)) {
    ...
    const BRepAdaptor_Surface anAS(aFF);
    const Standard_Real aTol2d = Min(anAS.UResolution(aTolF), anAS.VResolution(aTolF));
    //
    pFClass2d = 
        (BRepTopAdaptor_FClass2d*)myAllocator->Allocate(sizeof(BRepTopAdaptor_FClass2d));
    new (pFClass2d)BRepTopAdaptor_FClass2d(aFF, Max(aTol2d, Precision::PConfusion()));
    //
