...
static int VClipPlane (Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec)
{
  ...
  for (; anArgIter < theArgsNb; ++anArgIter)
  {
          ...
          || aChangeArg.StartsWith ("equation"))
    {
      ...
      {
        if (aSubPln->NextPlaneInChain().IsNull())
        {
          aSubPln->SetNextPlaneInChain (new Graphic3d_ClipPlane());
        }
        aSubPln = aSubPln->NextPlaneInChain();
      }
      aSubPln->SetNextPlaneInChain (Handle(Graphic3d_ClipPlane)());
