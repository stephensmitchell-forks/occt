...
The methods analogical to **GeomLib::AllowExtend(...)** that can be applied to a surface are called **GeomLib::AllowExtendUParameter(...)** and **GeomLib::AllowExtendVParameter(...)**.
...
Nevertheless, <b><i>calling of methods Period(), UPeriod() and VPeriod() for not-periodic elements is not recommended</i></b> and the behavior of these methods (for not-periodic elements) can be changed in future OCCT-versions.
