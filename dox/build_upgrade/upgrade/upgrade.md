Upgrade from Older OCCT Versions  {#build_upgrade__upgrade}
================================
...
It describes known issues that have been encountered during porting of OCCT and some applications and approaches that have helped to resolve these issues in known cases.
...
@note The current version of OCCT can be consulted in the file src/Standard/Standard_Version.hxx
...
@subsection upgrade_670_clipping_capping UpgradeObject-level clipping and capping algorithm
...
@subsection upgrade_670_default_view Default views are not created automatically
...
@subsection upgrade_670_ncollection NCollection_Set replaced by List collection
...
@subsection upgrade_680_connected_inter_objs Redesign of Connected Interactive Objects
...
@subsection upgrade_680_shift_concept Elimination of Projection Shift Concept
...
@subsection upgrade_690_viewer_init 3D Viewer initialization
...
@subsection upgrade_690_adaptor Changes in Adaptor3d_Curve class
...
@subsection upgrade_690_view Changes in V3d_View class
...
@subsection upgrade_700_legacy Removal of legacy persistence
...
@subsection upgrade_700_cdl Removal of CDL and WOK
...
The recommended method for building OCCT 7.x is \ref anchor_build_occt_win_cmake "CMake".
The alternative solution is to use project files generated by OCCT legacy tool **genproj**, see @ref build_upgrade__building_occt.

### Automatic upgrade

@note **Disclaimer** The automatic upgrade tool is provided as is, without warranty of any kind, and we explicitly disclaim any liability for possible errors that may appear due to use of this tool. 
...
11. Removes uses of obsolete macros *IMPLEMENT_DOWNCAST* and *IMPLEMENT_STANDARD_*..., except *IMPLEMENT_STANDARD_RTTIEXT*.
    ...
    @note If you plan to keep compatibility of your code with older versions of OCCT, add option <i>-compat</i> to avoid this change. See also \ref upgrade_occt700_cdl_compat "Preserving compatibility with OCCT 6.x"
...
\anchor upgrade_occt700_cdl_nocast
...
\anchor upgrade_occt700_cdl_compat
...
@subsection upgrade_700_bspline Separation of BSpline cache
...
@subsection upgrade_700_bop Structural result of Boolean operations
...
@subsection upgrade_700_extrema BRepExtrema_ExtCC finds one solution only
...
@subsection upgrade_700_sorttools Removal of SortTools package
...
@subsection upgrade_700_onscreen_color On-screen objects and ColorScale
...
@subsection upgrade_700_userdraw UserDraw and Visual3d
...
@subsection upgrade_700_local_context Deprecation of Local Context
...
@subsection upgrade_700_tkcaf Separation of visualization part from TKCAF
...
@subsection upgrade_700_quaternion Correction of interpretation of Euler angles in gp_Quaternion
...
@subsection upgrade_700_zoom Zoom Persistent Selection
...
@subsection upgrade_700_texture_mapping Correction of texture mapping of objects

Interaction of texture and environment texture is fixed. Textured objects have priority over the environment mapping.
Redundant enumerations <b>V3d_TypeOfSurface</b> and <b>Graphic3d_TypeOfSurface</b>, class <b>OpenGl_SurfaceDetailState</b>, corresponding methods from Graphic3d_CView, OpenGl_ShaderManager, OpenGl_View, V3d_View, V3d_Viewer are deleted.
...
* Obsolete Antialiasing API *V3d_View::SetAntialiasingOn()*. This method was intended to activate deprecated OpenGL functionality (GL_POLYGON_SMOOTH, GL_LINE_SMOOTH, GL_POINT_SMOOTH).
  Instead of old API, application should request MSAA buffers for antialiasing by assigning *Graphic3d_RenderingParams::NbMsaaSamples* property of structure returned by *V3d_View::ChangeRenderingParams()*.<br><br>
* *Prs3d_Drawer::ShadingAspectGlobal()* flag has been removed as not used. Corresponding calls can be removed safely from the application code.<br><br>
* ZClipping planes and ZCueing (methods <i>V3d_View::SetZClippingType()</i>, <i>V3d_View::SetZCueingOn()</i> and V3d_View::others).
  ZClipping planes can be replaced by general-purpose clipping planes (application should update plane definion manually).<br><br>
* 3D viewer printing API *V3d_View::Print()* has been removed. This functionality was available on Windows platforms only.
  ...
  Text resolution can be managed by rendering parameter *Graphic3d_RenderingParams::Resolution*, returned by *V3d_View::ChangeRenderingParams()*.<br><br>
